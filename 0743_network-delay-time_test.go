...
func Test_networkDelayTime2(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := networkDelayTime2(tt.args.times, tt.args.N, tt.args.K); got != tt.want {
...
func Test_networkDelayTime3(t *testing.T) {
	type args struct {
		times [][]int
		N     int
		K     int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := networkDelayTime3(tt.args.times, tt.args.N, tt.args.K); got != tt.want {
				t.Errorf("networkDelayTime3() = %v, want %v", got, tt.want)
			}
		})
	}
}
